...
CTVariablePredictionSample = namedtuple("CTVariablePredictionSample", ["tokens", "token_types",
                                                                     "node_types", "distance_matrices",
                                                                     "binning_vectors", "distance_names",
                                                                     # "func_name", "idx_func_tokens",
                                                                     "variable_name", "idx_var_tokens", # add these
...
class CTVariablePredictionDataset(CTBaseDataset):
    '''
    TODO desc
    '''
    def __init__(self, data_manager: CTPreprocessedDataManager, token_distances=None, max_distance_mask=None, num_sub_tokens=5,
                    num_sub_tokens_output=5, use_token_types=True, use_pointer_network=False, max_num_tokens=MAX_NUM_TOKENS):
        super().__init__(data_manager, token_distances, max_distance_mask, num_sub_tokens, 
                                use_token_types, use_pointer_network, max_num_tokens)
    ...
    def __next__(self):
        ...
        # Split variable name into sub tokens
        variable_name = sample.variable_name
        variable_name = variable_name[variable_name.rindex('.') + 1:] if '.' in variable_name else variable_name
        label = split_identifier_into_parts(variable_name)
        ...
        tmp_toks = sample.tokens.copy()
        idx_var_tokens_l = []
        i = 0
        while i < len(tmp_toks):
            ...
            # print("subtokens ", sample.tokens[i].sub_tokens,[self.word_vocab.reverse_lookup(st) for st in sample.tokens[i].sub_tokens])
            # print("encoded label: ", encoded_label, [self.word_vocab.reverse_lookup(st) for st in encoded_label])
            # print("'" * 40)
            ...
            label_idx = 0
            idx_var_tokens.append(i)

            while label_idx < min(self.num_sub_tokens, len(encoded_label)):
                if tmp_toks[v_i] is not None and v_j < len(tmp_toks[v_i].sub_tokens) and tmp_toks[v_i].sub_tokens[v_j] == encoded_label[
                    label_idx]:
                    ...
                    break
                elif at_least_one_match and tmp_toks[v_i] is not None and v_j >= len(tmp_toks[v_i].sub_tokens):
                    ...
                    if v_i >= len(tmp_toks):
                        ...
                        idx_var_tokens.append(v_i)
                else:
                    idx_var_tokens = []
                    at_least_one_match = False
                    break  # Cannot match variable name at this position
            i += 1
            if at_least_one_match:
                # Variable name was matched, exit the loop
                i = idx_var_tokens[-1]
                idx_var_tokens_l += idx_var_tokens
                idx_var_tokens = []
                i += 1

        # print("idx var tokens l:", idx_var_tokens_l)
        idx_var_tokens = idx_var_tokens_l

        if not idx_var_tokens:
            logger.warn(f"variable prediction - Could not find method name {variable_name} in token sequence, skipping sample")
            ...
            # Ensure to free memor
            del sample, cls_tokens, cls_token, variable_name, label, idx_func_tokens, idx_var_tokens, encoded_label
            return next(self)
        
        # print("subtokens 1: ", [self.word_vocab.reverse_lookup(tok) for token in sample.tokens for tok in token.sub_tokens] )
        # print("sourcespan: ", [token.source_span.substring(sample.stripped_code_snippet) if token.source_span is not None else None for token in sample.tokens])
        # print("code:\n", sample.stripped_code_snippet)

        # Remove any additional tokens if label is more than 1 token long
        # sample.tokens = [t for i, t in enumerate(sample.tokens) if i not in idx_var_tokens[1:]]
        # sample.token_mapping = [m for i, m in enumerate(sample.token_mapping) if i not in idx_var_tokens[1:]]
        # sample.tokens[idx_var_tokens[0]] = CTToken(
        #     [self.word_vocab[CLS_TOKENS[-1]]],
        #     RangeInterval.empty_interval(),
        #     self.token_type_vocab[UNKNOWN_TOKEN])

        for idx in idx_var_tokens:
            sample.tokens[idx] = CTToken(
                [self.word_vocab[CLS_TOKENS[-1]]],
                RangeInterval.empty_interval(),
                self.token_type_vocab[UNKNOWN_TOKEN])

        # print("subtokens 2: ", [self.word_vocab.reverse_lookup(tok) for token in sample.tokens for tok in token.sub_tokens] )

        # Replace label token with method name mask
        # idx_var_tokens = [idx_var_tokens[0]]  # only one label position left now

        transformed_sample = self.transform_sample_var(sample) # Changed transform_sample_var 
        
        assert len(label) > 0, \
            f"Variable name has to be comprised of at least 1 sub token, got {transformed_sample.variable_name}"

        if hasattr(self, 'word_vocab_labels'):
            # We have a separate vocabulary for method name tokens
            encoded_label = pad_or_truncate(sample.encoded_variable_name, self.num_sub_tokens_output,
        ...
        if self.use_pointer_network:
            ...
            #n_label_subtokens = (transformed_sample.pointer_pad_mask[idx_var_tokens] == True).sum().item()
            #assert n_label_subtokens == 1, f"Label should be only a masked label token, but was {transformed_sample.tokens[idx_var_tokens]}"
            
            # Mask all variable name
            for idx in idx_var_tokens:
                transformed_sample.pointer_pad_mask[idx] = False
            #idx_var_tokens = [idx_var_tokens[0]]

            # Remove vairable name tokens from extended vocabulary IDs such that they cannot be pointed to
            for idx in idx_var_tokens:
                idx_var_sub_tokens = transformed_sample.pointer_pad_mask[:idx].sum().item()
                del transformed_sample.extended_vocabulary_ids[idx_var_sub_tokens]
        ...
        return CTVariablePredictionSample(tokens=transformed_sample.tokens,
                                         ...
                                         variable_name=transformed_sample.variable_name,
                                         idx_var_tokens=idx_var_tokens,
                                         label=encoded_label,
    ...
    def collate_fn(self, batch: List[CTVariablePredictionSample]):
        collated_batch = super(CTVariablePredictionDataset, self).collate_fn(batch)
        ...
        for sample in batch:
            ...
            for idx in sample.idx_var_tokens:
...
class CTVariablePredictionNoPunctuation(CTVariablePredictionDataset):
    """
    ...
    def __init__(self, data_manager: CTPreprocessedDataManager, token_distances=None, max_distance_mask=None,
        ...
        super(CTVariablePredictionNoPunctuation, self).__init__(data_manager, token_distances, max_distance_mask,
    ...
    def __next__(self):
        sample = super(CTVariablePredictionNoPunctuation, self).__next__()
        ...
        idx_var_tokens_no_punctuation = []
        for idx_var_token in sample.idx_var_tokens:
            idx_var_tokens_no_punctuation.append(len([i for i in idx if i < idx_var_token]))
        ...
        return CTVariablePredictionSample(tokens_no_punctuation, token_types_no_punctuation,
                                         node_types_no_punctuation, distance_matrices_no_punctuation,
                                         sample.binning_vectors, sample.distance_names, 
                                         sample.variable_name, idx_var_tokens_no_punctuation,
                                         sample.label,
...
class CTVariablePredictionDatasetEdgeTypes(CTVariablePredictionDataset):
    """
    ...
    def __init__(self, data_manager: CTPreprocessedDataManager, token_distances=None, max_distance_mask=None,
        ...
        super(CTVariablePredictionDatasetEdgeTypes, self).__init__(data_manager, token_distances, max_distance_mask,
                                                                  num_sub_tokens, num_sub_tokens_output,
                                                                  use_pointer_network=use_pointer_network,
                                                                  max_num_tokens=max_num_tokens)

    def __next__(self):
        return super(CTVariablePredictionDatasetEdgeTypes, self).__next__()

    def collate_fn(self, batch: List[CTVariablePredictionSample]):
        collated_batch = super(CTVariablePredictionDatasetEdgeTypes, self).collate_fn(batch)
        seq_len = collated_batch.tokens.shape[1]
